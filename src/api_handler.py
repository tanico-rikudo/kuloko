...
class API:
    ...
    def load_urls(self):
        self.url_parts = {
            ...
            'margin':self.config.get('MARGIN_URL'),
            'assets':self.config.get('ASSERTS_URL'),
            'activeOrders':self.config.get('ACTIVEORDERS_URL'),
            'orders':self.config.get('ORDERS_URL'),
            'executions':self.config.get('EXECUTIONS_URL'),
            'latestExecutions':self.config.get('LATESTEXECUTIONS_URL'),
            'order':self.config.get('ORDER_URL'),
            'changeOrder':self.config.get('CHANGEORDER_URL'),
            'cancelOrder':self.config.get('CANCELORDER_URL'),
    ...
    def load_config(self):
        ...
        self.private_api = private_api_ini["DEFAULT"]
        logger.info('[DONE]Load private API Config. Mode=DEFAULT')
    ...
    def make_header(self,access_path, access_method,request_body=""):
        ...
        sign = hmac.new(
            bytes(self.private_api.get('API_SEC').encode('ascii')), 
            bytes(send_text.encode('ascii')), 
            hashlib.sha256).hexdigest()

        headers = {
            "API-KEY": self.private_api.get('API_KEY'),
    ...
    def post_data(self, target_url, headers=None, data=None):
        try:
            ...
            if data['status'] !=0:
                raise RequestError(data['messages'])
            logger.info("[DONE] Post Data. URL={0}".format(target_url))
        except RequestError as e:
            logger.error(e,exc_info=True)    
        except Exception as e:
            raise Exception(e)
...
class Margin(API):
    ...
    def fetch(self, return_type='json', since_time=None, *args, **kwargs):
        ...
        logger.info("[DONE] Fetch margin.  Return_type={0}".format(return_type))
        return margin
...
class Assets(API):
    ...
    def fetch(self, return_type='json', *args, **kwargs):
        ...
        logger.info("[DONE] Fetch assets.  Return_type={0}".format(return_type))
        return assets
...
class Orders(API):
    ...
    def fetch_by_orderId(self, orderId, return_type='json', *args, **kwargs):
        ...
        logger.info("[DONE] Fetch orders. orderId={0}, Return_type={1}".format(orderId, return_type))
        return orders
    ...
    def fetch_active(self, sym, return_type='json', *args, **kwargs):
        ...
        logger.info("[DONE] Fetch activeOrders. Sym={0}, Return_type={1}".format(self.sym, return_type))
        return activeOrders
...
class Executions(API):
    ...
    def fetch_by_id(self, orderId=None,executionId=None, return_type='json', *args, **kwargs):
        ...
        logger.info("[DONE] Fetch executions. orderId={0},  executionId={1},  Return_type={2}".format(orderId, executionId, return_type))
        return executions
    ...
    def fetch_latestExecutions(self,sym, count=100, return_type='json', *args, **kwargs):
        ...
        logger.info("[DONE] Fetch latestExecutions.  Sym={0}, Return_type={1}".format(sym, return_type))
...
class Order(API):
    ...
    def validate_order_params(self, reqBody):
        ...
        if reqBody["symbol"] not in eval(self.config.get("ALLOW_SYM")):
        ...
        if reqBody["side"] not in eval(self.config.get("EXEC_SIDE")):
        ...
        if reqBody["executionType"] not in eval(self.config.get("EXEC_TYPE")):
        ...
        if reqBody["timeInForce"] is None:
            if reqBody["executionType"] in ['MARKET','STOP']:
                ...
                logger.info("timeInForce is set automatically=FAK")
            if reqBody["executionType"] in ['LIMIT']:
                reqBody["timeInForce"] = 'FAS'
                logger.info("timeInForce is set automatically=FAS") 
        if type(reqBody["timeInForce"]) is not str:
            raise Exception("symbol must be string. you set type={0}".format(type(reqBody["symbol"])))
        if reqBody["timeInForce"] not in eval(self.config.get("TIME_IN_FORCE")):
        ...
        if reqBody["timeInForce"] in ["FOK",'FAS','SOK']:
            ...
            if reqBody["timeInForce"] == "SOK":
                if reqBody["symbol"] not in eval(self.config.get("LISTED_SYM")):
        ...
        if reqBody["losscutPrice"] is not None:
            if reqBody["symbol"] not in eval(self.config.get("LISTED_REV_SYM")):
        ...
        logger.info("[DONE] Order parameter validation: OK")
        return reqBody
    ...
    def validate_change_params(self, reqBody):
        ...
        if reqBody["orderId"] is None:
         ...
         # excl none
        reqBody = {_key :_val for _key, _val in reqBody.items() if _val is not None}
        logger.info("[DONE] Change order parameter validation: OK")
        return reqBody     
    ...
    def validate_cancel_params(self, reqBody):
        if reqBody["orderId"] is None:
         ...
         # excl none
        reqBody = {_key :_val for _key, _val in reqBody.items() if _val is not None}
        logger.info("[DONE] Cancel order parameter validation: OK")
        return reqBody       
    ...
    def do_order(self,return_type='json', *args, **order_kwargs):
        try:
            ...
            if order['status'] == 0 :
                logger.info("[DONE] Post order. OrderId={0}, reqBody={1}".format(order['data'], json.dumps(reqBody)))
            else :
                logger.error("Reject to post order. reqBody={0}".format(json.dumps(reqBody)))
            return order

        except Exception as e:
            logger.error("Fail to post order: {0}".format(e),exc_info=True)
    ...
    def do_change(self,return_type='json', *args, **order_kwargs):
        try:
            ...
            order = self.post_data(target_url,headers=headers,data=dumped_req_body)
            if order['status'] == 0 :
                logger.info("[DONE] Change order. OrderId={0}, reqBody={1}".format(order_kwargs["orderId"], json.dumps(reqBody)))
            else :
                logger.error("Rejject to Change order. OrderId={0}, reqBody={1}".format(order_kwargs["orderId"], json.dumps(reqBody)))
            return order

        except Exception as e:
            logger.error("Fail to change order: {0}".format(e),exc_info=True)
    ...
    def do_cancel(self,return_type='json', *args, **order_kwargs):
        try:
            ...
            order = self.post_data(target_url,headers=headers,data=dumped_req_body)
            if order['status'] == 0 :
                logger.info("[DONE] Cancel order. OrderId={0}, reqBody={1}".format(order_kwargs["orderId"], json.dumps(reqBody)))
            else :
                logger.error("Reject to cancel order. OrderId={0}, reqBody={1}".format(order_kwargs["orderId"], json.dumps(reqBody)))
            return order

        except Exception as e:
            logger.error("Fail to cancel order: {0}".format(e),exc_info=True)
