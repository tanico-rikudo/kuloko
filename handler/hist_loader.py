...
class HistDataHandler:
    ...
    def set_config(self):
        self.remote_end_point = os.path.join(self.general_config.get('ENDPOINT_URL'),
    ...
    def get_file_hist(self, sym, kind, int_date, is_save=False, mode="auto"):
        ...
        if df is not None:
            ...
            df['timestamp'] = df['timestamp'].apply(
                lambda x: dl.dt_to_strYMDHMSF(dl.str_utc_to_dt_offset(x, is_Z=False, is_T=False)))
    ...
    def bulk_load(self, sym, kind, since_int_date, until_int_date, is_save=True, mode='auto'):
        ...
        dfs = Parallel(n_jobs=self.n_usable_core)(
            delayed(self.get_file_hist)(sym, kind, _target_date, is_save, mode) for _target_date in target_dates)
