...
class HistDataHandler:
    ...
    def set_config(self):
        self.remote_end_point = os.path.join(
            self.general_config.get("ENDPOINT_URL"),
            self.general_config.get("HISTDATA_URL"),
        )
    ...
    def get_file_hist(self, sym, kind, int_date, is_save=False, mode="auto"):
        ...
        if mode == "local":
            ...
            self._logger.info(
                "[DONE] Get remote hist data. Path={0}".format(remote_path)
            )
        elif mode == "auto":
            if not os.path.isfile(local_path):
                self._logger.info(
                    "No local hist data. Try: remote.  path{0}".format(local_path)
                )
                df = self.download_remote_hist(remote_path, save_path=save_path)
                self._logger.info(
                    "[DONE] Get remote hist data. Path={0}".format(remote_path)
                ...
                df = self.read_local_hist(local_path)
                self._logger.info(
                    "[DONE] Get local hist data. Path={0}".format(local_path)
                )
    ...
    def download_remote_hist(self, url, save_path=None):
        ...
        try:
            ...
            if response.headers["Content-Type"] != "text/csv":
                raise Exception(
                    "Response content type is invalid. Type={0}".format(
            ...
            self._logger.error(
                "Fail to get historical data. url={0}:{1}".format(url, e)
            )
            return None
        ...
        if save_path is None:
            ...
            try:
                ...
                self._logger.error(
                    "Fail to save historical data. Url={0}:{1}".format(save_path, e)
                )
            finally:
                return data
    ...
    def read_local_hist(self, read_local_path):
        ...
        try:
            ...
            self._logger.warning(
                "[Fialure] Reading hist data. Path={0}:{1} ".format(read_local_path, e)
            )
        return df
    ...
    def bulk_load(
        self, sym, kind, since_int_date, until_int_date, is_save=True, mode="auto"
    ):
        target_dates = dl.get_between_date(since_int_date, until_int_date)
        dfs = Parallel(n_jobs=self.n_usable_core)(
            delayed(self.get_file_hist)(sym, kind, _target_date, is_save, mode)
