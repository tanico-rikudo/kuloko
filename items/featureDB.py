...
class FeatureDB(Item):
    ...
    def download_update_trade(self, symbol, until_date=None):
        ...
        if df.shape[0] == 0:
            self.logger.warning(f"[Failure] All Download trade. Sym={symbol}, Date={since_date}~{until_date}")
            return
        fetch_dates = set([int(_date) for _date in list(df.index.dt.strftime("%Y%m%d"))])
        target_dates = set(dl.get_between_date(since_date, until_date))
        left_dates = target_dates - fetch_dates
        if len(left_dates) > 0:
            self.logger.warning(f"[Failure] Download trade. Sym={symbol}, Failure Date={left_dates}")
        else:
    ...
    def create_volatility(self, symbol, until_date=None):
        ...
        daily_close_trade = trades.price.resample('T', label='left', closed='left').ohlc().loc[:, "close"]
        ...
        # calc volatility
        daily_close = self.calc_daily_volatility(daily_close, VOLATILITY_VAR_DAYS, VOLATILITY_DAYS, close_time="00:00")
        ...
        self.hd.mongo_db.dao.insert_many(insert_json)
    ...
    def calc_daily_volatility(daily_close, variance_days, volatility_days, close_time="00:00"):
        ...
        df_date["datetime"] = pd.to_datetime(df_date["date"] + close_time, format="%Y%m%d %H:%M")
        del df_date["date"]

        df_close = pd.merge_asof(df_date, daily_close, on="datetime", direction="nearest")
        df_close["volatility"] = df_close.pct_change().rolling(variance_days).std() * np.sqrt(volatility_days)
        return df_close
