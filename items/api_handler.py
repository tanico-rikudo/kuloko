...
class API:
    ...
    def load_config(
        ...
        if private_api_ini is None:
            self.private_api_config = cm.load_ini_config(
    ...
    def convert_shape(self, raw_data, return_type):
        if return_type is "raw":
            ...
            raise InvalidArgumentError(
                "Cannot accept in venueStatus. Return_type={0}".format(return_type)
            )
...
class Orderbook(API):
    ...
    def fetch(self, depth=None, return_type="dataframe", *args, **kwargs):
        ...
        self._logger.info(
            "[DONE] Fetch orderbook. Depth={0}, Return_type={1}".format(
                depth, return_type
            )
        )
        return data
...
class Ticks(API):
    ...
    def convert_shape(self, raw_data, return_type):
        ...
        if return_type is "raw":
            ...
            data["symbol"] = raw_data["data"][0]["symbol"]
...
class Trade(API):
    ...
    def fetch(self, return_type="json", since_time=None, *args, **kwargs):
        ...
        self._logger.debug(f"Trade Query.. {target_url}")
        trade = self.fetch_data(target_url)

        latest_time = dl.str_utc_to_dt_offset(
            trade["data"]["list"][0]["timestamp"], self.tz
        )
        since_time = (
            dl.str_utc_to_dt_offset(trade["data"]["list"][-1]["timestamp"], self.tz)
    ...
    def convert_shape(self, raw_data, return_type, since_time):
        ...
        if return_type is "raw":
            ...
            for _trade in raw_data["data"]["list"]:
                _trade["sym"] = self.sym
                _trade["timestamp"] = dl.str_utc_to_dt_offset(
                    _trade["timestamp"], self.tz
                )
...
class Margin(API):
    ...
    def convert_shape(self, raw_data, return_type):
        responsetime_dt = dl.str_utc_to_dt_offset(
            raw_data["responsetime"], self.tz, is_Z=True, is_ms=True
        )
...
class Orders(API):
    ...
    def fetch_by_orderId(self, orderId, return_type="json", *args, **kwargs):
        ...
        self._logger.info(
            "[DONE] Fetch orders. orderId={0}, Return_type={1}".format(
                orderId, return_type
            )
        )
        return orders
    ...
    def fetch_active(self, sym, return_type="json", *args, **kwargs):
        parameters = {"symbol": sym, "page": 1, "count": 10}
        target_url = self.get_url("activeOrders")
        headers = self.make_header(
            self.url_parts["activeOrders"].split("/private")[1], "GET"
        )
        ...
        self._logger.info(
            "[DONE] Fetch activeOrders. Sym={0}, Return_type={1}".format(
                self.sym, return_type
            )
        )
        return activeOrders
        ...
        if return_type is "raw":
            ...
            if len(raw_data["data"]) > 0:
                for _order in raw_data["data"]["list"]:
                    ...
                    _order["timestamp"] = dl.str_utc_to_dt_offset(
                        _order["timestamp"], self.tz
                    )
                    data.append(_order)
            if return_type in "json":
                for i in range(len(data)):
...
class Executions(API):
    ...
    def fetch_by_id(
        ...
        if len(parameters.keys()) > 1:
            raise Exception(
                "Cannot set both orderId and executionId. Needs to set either key."
            )
        if len(parameters.keys()) == 0:
            raise Exception("Needs to set either orderId or executionId ")
        target_url = self.get_url("executions")
        headers = self.make_header(
            self.url_parts["executions"].split("/private")[1], "GET"
        )
        ...
        self._logger.info(
            "[DONE] Fetch executions. orderId={0},  executionId={1},  Return_type={2}".format(
                orderId, executionId, return_type
            )
        )
        return executions
    ...
    def fetch_latestExecutions(
        ...
        headers = self.make_header(
            self.url_parts["latestExecutions"].split("/private")[1], "GET"
        )
        latestExecutions = self.fetch_data(
            target_url, headers=headers, params=parameters
        )
        latestExecutions = self.convert_shape(latestExecutions, return_type)

        self._logger.info(
            "[DONE] Fetch latestExecutions.  Sym={0}, Return_type={1}".format(
                sym, return_type
            )
        )
        ...
        if return_type is "raw":
            ...
            if not len(raw_data["data"]) == 0:
                for _execution in raw_data["data"]["list"]:
                    ...
                    _execution["timestamp"] = dl.str_utc_to_dt_offset(
                        _execution["timestamp"], self.tz
                    )
...
class Order(API):
    ...
    def validate_order_params(self, reqBody):
        ...
        if not utils.is_type(reqBody["symbol"], str):
            raise OrderParamException(
                "symbol must be string. you set type={0}".format(
                    type(reqBody["symbol"])
                )
            )
        ...
        if reqBody["symbol"] not in eval(self.general_config.get("ALLOW_SYM")):
            raise OrderParamException(
                "Order validation error. Invalid Sym={0}".format(reqBody["symbol"])
            )
        ...
        if not utils.is_type(reqBody["side"], str):
            raise OrderParamException(
                "side must be string. you set type={0}".format(type(reqBody["side"]))
            )
        ...
        if reqBody["side"] not in eval(self.general_config.get("EXEC_SIDE")):
            raise OrderParamException(
                "Order validation error. Invalid Side={0}".format(reqBody["side"])
            )
        ...
        if not utils.is_type(reqBody["executionType"], str):
            raise OrderParamException(
                "executionType must be string. you set type={0}".format(
                    type(reqBody["executionType"])
                )
            )
        ...
        if reqBody["executionType"] not in eval(self.general_config.get("EXEC_TYPE")):
            raise OrderParamException(
                "Order validation error. Invalid executionType={0}".format(
                    reqBody["executionType"]
                )
            )
        ...
        if not utils.is_type(reqBody["timeInForce"], str):
            raise OrderParamException(
                "timeInForce must be string. you set type={0}".format(
                    type(reqBody["timeInForce"])
                )
            )
        if reqBody["timeInForce"] not in eval(self.general_config.get("TIME_IN_FORCE")):
            raise OrderParamException(
                "Order validation error. Invalid timeInForce={0}".format(
                    reqBody["timeInForce"]
                )
            )
        if reqBody["timeInForce"] == "FAK":
            if reqBody["executionType"] not in ["MARKET", "STOP"]:
                raise OrderParamException(
                    "Order validation error. executionType must be in MARKET or STOP when TimeInForce=FAK, but you set executionType={0}".format(
                        reqBody["executionType"]
                    )
                )
        if reqBody["timeInForce"] in ["FOK", "FAS", "SOK"]:
            if reqBody["executionType"] not in ["LIMIT"]:
                raise OrderParamException(
                    "Order validation error. executionType must be in LIMIT when TimeInForce={0}, but you set executionType={1}".format(
                        reqBody["timeInForce"], reqBody["executionType"]
                    )
                )
            if reqBody["timeInForce"] == "SOK":
                if reqBody["symbol"] not in eval(self.general_config.get("LISTED_SYM")):
                    if reqBody["symbol"] != "BTC_JPY":
                        raise OrderParamException(
                            "Order validation error. timeInForce can be set SOK when symbol is in spot symbols or BTC_JPY, you set={0}".format(
                                reqBody["symbol"]
                            )
                        )
        ...
        if reqBody["executionType"] in ["LIMIT", "STOP"]:
            ...
            if reqBody["price"] < 0.0:
                raise OrderParamException(
                    "Price must be >0. you set = {0}".format(reqBody["price"])
                )
        else:
            reqBody["price"] = None
        ...
        if reqBody["losscutPrice"] is not None:
            if reqBody["symbol"] not in eval(self.general_config.get("LISTED_REV_SYM")):
                raise OrderParamException(
                    "losscutPrice can be set when symbol is in ONLY Leverage symbols={0}".format(
                        self.general_config.get("LISTED_REV_SYM")
                    )
                )
            ...
            if not (
                ...
                raise OrderParamException(
                    "losscutPrice must be int/float. you set type={0}".format(
                        type(reqBody["losscutPrice"])
                    )
                )
            if reqBody["losscutPrice"] <= 0.0:
                raise OrderParamException(
                    "losscutPrice must be >0. you set = {0}".format(
                        reqBody["losscutPrice"]
                    )
                )
        else:
        ...
        if reqBody["size"] <= 0.0:
            raise OrderParamException(
                "size must be >0. you set = {0}".format(reqBody["size"])
            )
        ...
        if reqBody["cancelBefore"] is not None:
            if not utils.is_type(reqBody["cancelBefore"], bool):
                raise OrderParamException(
                    "cancelBefore must be bool. you set type={0}".format(
                        type(reqBody["cancelBefore"])
                    )
                )
            if reqBody["cancelBefore"] is True:
                if not reqBody["executionType"] is "MARKET":
                    raise OrderParamException(
                        "executionType must be MARKET. You set={0}".format(
                            reqBody["executionType"]
                        )
                    )
                if not reqBody["timeInforce"] is "FAK":
                    raise OrderParamException(
                        "timeInforce must be FAK. You set={0}".format(
                            reqBody["timeInforce"]
                        )
                    )
                if not reqBody["side"] is "SELL":
                    raise OrderParamException(
                        "side must be SELL. You set={0}".format(reqBody["side"])
                    )
                if reqBody["symbol"] in eval(self.general_config.get("LISTED_REV_SYM")):
                    raise OrderParamException(
                        "losscutPrice CANNOT be set TRUE when symbol is in Leverage symbols={0}".format(
                            self.general_config.get("LISTED_REV_SYM")
                        )
                    )
            else:
    ...
    def validate_change_params(self, reqBody):
        ...
        if not utils.is_type(reqBody["orderId"], str):
            raise OrderParamException(
                "orderId must be str. you set type={0}".format(type(reqBody["orderId"]))
            )
    ...
    def validate_cancel_bulk_params(self, reqBody):
        ...
        if not utils.is_type(reqBody["symbols"], list):
            raise OrderParamException(
                "symbols must be list. you set type={0}".format(
                    type(reqBody["symbols"])
                )
            )
        not_allowed_syms = list(
            set(reqBody["symbols"]) - set(eval(self.general_config.get("ALLOW_SYM")))
        )
        if len(not_allowed_syms) > 0:
            raise OrderParamException(
                "Following symbols are not allowed: {0}".format(not_allowed_syms)
            )
        ...
        if not utils.is_type(reqBody["desc"], bool):
            raise OrderParamException(
                "desc must be string. you set type={0}".format(type(reqBody["desc"]))
            )
    ...
    def do_order(self, return_type="json", *args, **order_kwargs):
        ...
        headers = self.make_header(
            self.url_parts["order"].split("/private")[1],
            "POST",
            request_body=dumped_req_body,
        )

        res = self.post_data(target_url, headers=headers, data=dumped_req_body)
        if res["status"] == 0:
            self._logger.info(
                "[DONE] Post order. OrderId={0}, reqBody={1}".format(
                    res["data"], json.dumps(reqBody)
                )
            )
            success = True
        else:
            self._logger.error(
                "Reject to post order. Return={0}, reqBody={1}".format(
                    res, json.dumps(reqBody)
                )
            )
            success = False
        return res, success
    ...
    def do_change(self, return_type="json", *args, **order_kwargs):
        ...
        dumped_req_body = json.dumps(reqBody)
        headers = self.make_header(
            self.url_parts["changeOrder"].split("/private")[1],
            "POST",
            request_body=dumped_req_body,
        )
        res = self.post_data(target_url, headers=headers, data=dumped_req_body)
        if res["status"] == 0:
            self._logger.info(
                "[DONE] Change order. OrderId={0}, reqBody={1}".format(
                    orderId, json.dumps(reqBody)
                )
            )
            success = True
        else:
            self._logger.error(
                "Reject to Change order. OrderId={0}, Return={1}, reqBody={2}".format(
                    orderId, res, json.dumps(reqBody)
                )
            )
            success = False
        return res, success
    ...
    def do_cancel(self, return_type="json", *args, **order_kwargs):
        ...
        dumped_req_body = json.dumps(reqBody)
        headers = self.make_header(
            self.url_parts["cancelOrder"].split("/private")[1],
            "POST",
            request_body=dumped_req_body,
        )
        res = self.post_data(target_url, headers=headers, data=dumped_req_body)
        if res["status"] == 0:
            self._logger.info(
                "[DONE] Cancel order. OrderId={0}, reqBody={1}".format(
                    order_kwargs["orderId"], json.dumps(reqBody)
                )
            )
            success = True
        else:
            self._logger.error(
                "Reject to cancel order. OrderId={0}, Return={1}, reqBody={2}".format(
                    order_kwargs["orderId"], res, json.dumps(reqBody)
                )
            )
            success = False
        return res, success
    ...
    def do_bulk_cancel(self, return_type="json", *args, **order_kwargs):
        ...
        dumped_req_body = json.dumps(reqBody)
        headers = self.make_header(
            self.url_parts["cancelBulkOrder"].split("/private")[1],
            "POST",
            request_body=dumped_req_body,
        )
        res = self.post_data(target_url, headers=headers, data=dumped_req_body)
        if res["status"] == 0:
            orderIds = res["data"]
            self._logger.info(
                "[DONE] Cancel bulk order. OrderIds={0}, reqBody={1}".format(
                    orderIds, json.dumps(reqBody)
                )
            )
            success = True
        else:
            self._logger.error(
                "Reject to cancel bulk order.Return={0}, reqBody={1}".format(
                    res, json.dumps(reqBody)
                )
            )
            success = False
